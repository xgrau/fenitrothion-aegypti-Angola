...
# define sample list
sample_list=$1
echo "Samples and num specimens per sample:"
cat ${sample_list}
...
vcf_freebayes () {
   ...
   while read -a sample ; do
		s=${sample[0]} # sample
		n=${sample[1]} # num specimens in sample
		p=${sample[2]} # ploidy
		...
		if [ ! -f "${dir}/variants_gvcf_transcripts/${s}.raw.vcf.gz" ] ; then
			...
			freebayes --region {} -f ${ref} ${dir}/alignments_transcripts/${s}.bam --ploidy ${p} --pooled-discrete --pooled-continuous \
