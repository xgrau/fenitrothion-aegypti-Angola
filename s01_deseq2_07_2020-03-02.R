...
si      = "Aedaeg"
outfolder = "results_de/"
gfffile = "data_genome/Aedaeg_long.annot.gff"
txgdict = "data_genome/Aedaeg_long.cds.csv"
nmgdict = "data_genome/Aedaeg_genes.description.csv"
...
ex_dnc_dimred = dimredfun(
  matriu = ex_dnc_st, outputname = paste(outfolder,"de_all",sep=""), varname = "standardized log norm counts", 
...
ex_dnc_kmeans = kmeansfun(
  matrix=ex_dnc_st,
  outputprefix = paste(outfolder,"de_all",sep=""), 
...
for (com in complis) {
  ...
  ex_lis_i = volcanoexp(
    table=ex_res_i,plotname=tici,fileprefix=paste(outfolder,"de_co",sep=""),
  ...
  pdf(file=paste(outfolder,"de_co.",tici,"_topgenes.pdf",sep=""),height=12,width=8)
  par(mfrow=c(6,4))
  ...
  save(list=c("ex_dat_i","ex_lis_i","ex_dnc_i","ex_res_i","sf_i"), 
       file=paste(outfolder,"de_co.",tici,".data.RData",sep=""))
...
for (com in complis) {
  ...
  tici   = paste(com[5],".",com[2],"-",com[3],sep="")
  load(file = paste(outfolder,"de_co.",tici,".data.RData",sep=""))
  ...
  hygeofun(list_interest=ex_lis_i$tx_sp,annotation=pannu,gene_col="transcript",ano_col="domain",
           outputname=paste(outfolder,"de_co",sep=""),
...
pdf(file=paste(outfolder,"de_all.gene_overlaps.pdf",sep=""),height=4,width=4)
for (cop in complis_pairs) {
  ...
  load(paste(outfolder,"de_co.",pai.tici,".data.RData",sep=""))
  ex_lis_joini[[1]]$genes_sp = ex_lis_i$genes_sp
  ex_lis_joini[[1]]$genes_sn = ex_lis_i$genes_sn
  load(paste(outfolder,"de_co.",paj.tici,".data.RData",sep=""))
...
for (com in complis) {
  ...
  # load expression data for comparison
  tici = paste(com[5],".",com[2],"-",com[3],sep="")
  load(paste(outfolder,"de_co.",tici,".data.RData",sep=""))
  ...
  pdf(file=paste(outfolder,"de_co.",tici,"_chromosomalbias.pdf",sep=""),height=18,width=6)
  ...
  # save table
  write.table(ex_gff_i,file=paste(outfolder,"de_co.",tici,"_chromosomalbias.csv",sep=""),
              quote = F,row.names = F)
...
for (com in complis) {
  ...
  write.table(ex_res_i,file=paste(outfolder,"session.deseq_difexp",tici,".csv",sep=""),
...
write.table(ex_res_tot,file=paste(outfolder,"session.deseq_difexp.csv",sep=""),
            quote = T,row.names = F,sep = "\t")

save.image(paste(outfolder,"session.deseq_all.RData",sep=""))
